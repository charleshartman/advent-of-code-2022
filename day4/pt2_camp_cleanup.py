# pt2_camp_cleanup.py -- adventOfCode2022 - day4 - part1
...
def overlap_checker(items: list) -> bool:
    if (
        (items[0][0] >= items[1][0] and items[0][1] <= items[1][1])
        or (items[1][0] >= items[0][0] and items[1][1] <= items[0][1])
        or (items[0][0] >= items[1][0] and items[0][0] <= items[1][1])
        or (items[0][1] >= items[1][0] and items[0][1] <= items[1][1])
    ):
        return True
